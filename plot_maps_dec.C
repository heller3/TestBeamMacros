...
int run_boundaries[2] ={3150,3150};// {3150,3150};
const int nchan=36;//4;//36;
const int nlgad=16;//3;//16;
float hitThres=15.;
TChain * t;
TFile * outRootFile;
...
//vector<float> y_div = {20.4,20.77,21.24,21.7};
vector<float> y_div = {20.5,20.95,21.4,21.85};
vector<float> x_div = {0.3,0.77,1.24,1.7};
...
for(uint ib = 0; ib < base_sel.size(); ib++){
	for(uint ie = 0; ie < eff_sel.size(); ie++){
		...
		name = Form("h_eff_uniformity_%i_%i",ib,ie);
		v_eff_uniformity.push_back(new TH1F(name,name,90,0,1.));
		name = Form("h_den_uniformity_%i_%i",ib,ie);
...
for(int i=0;i<nentries;i++){
	...
	if(ntracks!=1 || chi2 > 15 || nplanes<15 /*|| skippedEventsC != 0*/) continue;
	...
	float x_adjust; 
	float y_adjust;
	if(!scope){x_adjust = x_corr[3]+1.2-0.6;
				y_adjust = y_corr[3]-0.3+0.05; }
	else{x_adjust=x_dut[2];y_adjust=y_dut[2];}

	
	 pair<float,float> rotated = Rotate(x_adjust,y_adjust,0.2);//Rotate(x_adjust,y_adjust,0.28);
	...
	if(ptkindex>=0 && amp[index]<880){
		float delta_t = -LP2_20[index]+LP2_20[ptkindex];
...
float GetEff(TH3F * h3, int x_lo, int x_hi, int y_lo, int y_hi, int den){
...
bool GoodPhotekHit(){
	...
	else if(run>2900&&run<3700) return true;
	else return false;
...
void InitBranches(){
  ...
  t->SetBranchStatus("maxplanes", 1);
  t->SetBranchStatus("skippedEvents", 1);
  t->SetBranchStatus("skippedEventsC", 1);
  t->SetBranchStatus("missedHits", 1);
  t->SetBranchAddress("maxplanes", &maxplanes);
  t->SetBranchAddress("skippedEvents", &skippedEvents);
  t->SetBranchAddress("skippedEventsC", &skippedEventsC);
  t->SetBranchAddress("missedHits", &missedHits);

  if(!scope){
  t->SetBranchStatus("nplanes", 1);
  ...
  t->SetBranchStatus("risetime", 1);
    ...
    t->SetBranchAddress("LP2_20", &LP2_20);	
